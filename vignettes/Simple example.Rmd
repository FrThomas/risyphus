---
title: "How to use risyphus: A simple example"
author: "Fridtjof Thomas"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{How to use risyphus: A simple example}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---



---

>This is a test document only. Content will be added soon.

## Outline

This simple example will show:

* how to use a function in risyphus.

* how to get the risyphus functionality into html-output using Rmarkdown.


## Test image

Here we add an image: 

![Alt text](Vignettes_images/test_link_image.png)



Can this been done within R?  Let's check it out!

```{r echo = TRUE, message=FALSE, fig.height=7, fig.width=7}
library(imager)
image_test <- load.image('Vignettes_images/test_link_image.png')
plot(image_test, axes=FALSE)


```


Using a function for knitr directly:

```{r out.height=300, out.width=300}
library(knitr)
include_graphics(path = 'Vignettes_images/test_link_image.png')


```



Or like this?

```{r }
library(knitr)
include_graphics(path = 'Vignettes_images/test_link_image.png', dpi = 200)


```


Or like this?

```{r out.width="50%"}
library(knitr)
include_graphics(path = 'Vignettes_images/test_link_image.png')


```


## Used packages

List packages



*****

End of document

*****
